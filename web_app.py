...
def pick_quad_point(mug_img, x, y, which_point, q1, q2, q3, q4):
    ...
    pts[idx] = val
    return pts[0], pts[1], pts[2], pts[3], rx, ry, f"已设置点{which_point}: {val}（可用下方微调按钮修正）"
...
def run_mockup(mug_img, art_img, quad_mode, q1, q2, q3, q4, warp_mode, mesh_cols, mesh_rows, curve_strength, shading_strength, edge_fade, projection_strength, occlusion_strength):
    ...
    out_dir = Path("output/web"); out_dir.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory() as td:
        tdp = Path(td)
        art_path = tdp / "art.png"; art_img.save(art_path)
        mug_path = None
        if mug_img is not None:
            mug_path = tdp / "mug.png"; mug_img.save(mug_path)
        out_path = out_dir / "web_mockup.png"; debug_path = out_dir / "web_quad_debug.png"
        generate_mockup(mug=mug_path, artwork=art_path, out=out_path, quad=[q1,q2,q3,q4], quad_mode=quad_mode, warp_mode=warp_mode,
...
with gr.Blocks(title="Mug Mockup Web UI", css=css) as demo:
    ...
    with gr.Row(equal_height=True):
        with gr.Column(scale=5):
            ...
            with gr.Accordion("坐标助手（防点歪）", open=True, elem_classes=["compact"]):
                gr.Markdown("先点击粗定位，再用微调按钮 ±X/±Y 修正，避免鼠标点歪。")
                with gr.Row():
                ...
                with gr.Row():
                    x_slider = gr.Slider(0,1,value=0.33,step=0.0005,label="当前点X（相对坐标）")
                    y_slider = gr.Slider(0,1,value=0.34,step=0.0005,label="当前点Y（相对坐标）")
                with gr.Row():
                ...
                pick_msg = gr.Textbox(label="坐标状态", lines=1)

            with gr.Accordion("参数面板（含中文注释）", open=True, elem_classes=["compact"]):
                gr.Markdown("- **warp**：变形算法；**cylindrical** 适合杯子弧面，**mesh** 适合复杂局部形变，**perspective** 适合近似平面。")
                with gr.Row():
                ...
                with gr.Row():
                    mesh_cols = gr.Slider(8,40,value=20,step=1,label="mesh cols（网格列数，越大越细）")
                    mesh_rows = gr.Slider(8,40,value=16,step=1,label="mesh rows（网格行数，越大越细）")
                with gr.Row():
